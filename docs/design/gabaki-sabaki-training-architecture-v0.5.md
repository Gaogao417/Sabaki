...
1. 修改 TrainingTask 类型；
2. 移除 `TrainingTaskKind` / `TrainingTaskSource`；
3. 增加 `origin`；
4. Problem-like 字段并入 Task：prompt / goal / passRule / referenceLines /
   problemArea；
5. 修改 training_tasks migration；
6. 写 mapper 测试：旧 source 字段如何映射到新 origin。

## Phase 2：Workbench Mode 骨架

1. `WorkbenchTab.phase` 改为 `WorkbenchTab.mode`；
2. 增加 `WorkbenchMode = play | problem | recall | analysis`；
3. 实现 `workbenchTabService.openTask`；
4. 移除 `openGameTab / openProblemTab / openSnapshotProblemTab` 的新代码依赖；
5. UI 选择 panel 改成按 mode 渲染；
6. WorkbenchTab 增加 playerConfig。

## Phase 3：taskImportService

1. 实现 importLocalSgf；
2. 实现 import101Problem skeleton；
3. 实现 createTaskFromSnapshot；
4. 实现 createTaskFromBadMove；
5. 所有输出统一为 TrainingTask。

## Phase 4：Attempt + Submit + Recall

1. 实现 attemptService；
2. Play / Problem 落子写 Attempt；
3. 实现 aiMoveService；
4. Play 支持黑白 human / ai 配置；
5. Problem 支持 opponent self / ai 配置；
6. Problem AI 用 problemArea / analysis area 限制落子；
7. Submit 冻结 Attempt；
8. 建立 RecallSession，绑定 attemptId；
9. 切换 mode = recall。

## Phase 5：MoveEvaluation / BadMove

1. 实现 evaluationRules；
2. 实现 playTrainingMonitor；
3. pending evaluation；
4. analysis update 补齐；
5. major/severe BadMove；
6. 超时 failed。

## Phase 6：RecallCheckpoint

1. 实现 recallCheckpointService；
2. bad move 命中 checkpoint；
3. correction draft；
4. reveal AI candidates；
5. save comment；
6. resume Recall。

## Phase 7：Analysis / Global Snapshot

1. 实现 AnalysisModePanel；
2. 实现 AnalysisContext；
3. bad move list / comments / AI candidates；
4. snapshotService.captureSnapshotInput 支持任意 WorkbenchMode；
5. taskImportService.createTaskFromSnapshot；
6. snapshotFromCurrentContext → openTask(newTaskId, mode:'problem')。

## Phase 8：Review

1. 修改 review_schedule：直接引用 taskId；
2. reviewService.getDueItems；
3. openDueItem → openTask；
4. updateScheduleAfterResult。

## Phase 9：清理 legacy

1. 减少旧 `mode='problem'` 依赖；
2. 旧入口改成 adapter / service 代理；
3. 清理 source_kind / item_type / snapshot_problem 分支；
4. 删除不再使用的 openSnapshotProblemTab / openRecallSegmentTab。
...
```text
1. types/task.ts 新 TrainingTask + TaskOrigin
2. types/tab.ts WorkbenchMode / WorkbenchTab
3. workbenchStore
4. trainingRuntimeStore
5. trainingRepository skeleton + migrations
6. workbenchTabService.openTask
...
3. openTask 跑通 Play / Problem 两种 UI
```
...
1. attemptService
2. training_attempts
3. submit → recall skeleton
4. recall_sessions 绑定 attemptId
```
...
1. AnalysisModePanel
2. AnalysisContext
3. snapshotService 支持任意 WorkbenchMode
4. createTaskFromSnapshot
5. ReviewSchedule taskId 化
```
