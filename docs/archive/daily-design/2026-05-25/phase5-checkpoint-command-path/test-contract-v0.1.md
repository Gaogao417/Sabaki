...
| `docs/ui_ux/` | not used for this behavior contract | No visual, layout, token, or copy change is designed here. Existing control placement is treated as current UI evidence only. |
...
- `checkpoint_commenting`：comment 保存/编辑阶段；本 step 的 `saveCheckpointComment` 命令可以在内部使用该 substate 做 guard，但最终保存后应回到 `normal`。
...
| `reference/current` | reveal AI 可读取 MoveEvaluation/reference candidates；Recall panel可显示原线/AI线 | 只读。 |
...
-> flowService validates recall checkpoint guard with resolveTransition(event='commentCheckpoint')
...
- active tab remains `mode='recall'`, `recallSubstate='normal'`
- source Attempt protected fields remain unchanged
...
- `workbenchStore.updateTab(tabId, {recallSubstate})` to move within Recall substate only.
...
- 不得修改 source Attempt `userLine`、`result`、`status`，也不得通过 repository protected-field patch 间接修改。
...
| P5CP-T02 | STATE | MUST_AUTOMATE | RED | Real flowService submit correction + reveal AI updates repository/runtime/workbench substate through real checkpoint service. | 证明 command path forward state。 | 只测 mock service call，无法证明 substate 和 runtime 回流。 |
| P5CP-T03 | STATE | MUST_AUTOMATE | RED | Real flowService save comment command persists independent MoveComment, resumes Recall, clears active checkpoint, leaves source Attempt unchanged. | 覆盖 Phase 5 comment/resume completion. | comment 被嵌入 Attempt 或 checkpoint 不完整完成。 |
| P5CP-T04 | STATE | MUST_AUTOMATE | RED | Real flowService skip command marks checkpoint skipped, advances RecallSession, clears runtime, keeps mode recall/substate normal. | 覆盖跳过 checkpoint 的 completion path。 | skip 清 runtime 但 tab 仍 stuck in checkpoint substate。 |
...
| P5CP-T06 | SIDE_EFFECT | MUST_AUTOMATE | GREEN | Existing recallCheckpointService baseline remains the source for service/repository transition behavior. | 避免本 step 重写已通过 service tests。 | 如果 service baseline 缺失，flow tests 会承担过多责任。 |
...
| P5CP-T08 | UI_BEHAVIOR | MANUAL_ACCEPTANCE | DEFERRED | Visible comment editor/save control -> `onSaveCheckpointComment({content})` mapping. | 当前 in-scope code has no rendered comment save control evidence. | 未来补 UI 时 signature drift。Approved reason: step1 closes shell/flow command path; exit condition: comment editor lands in panel scope. |
...
| RED | P5CP-T02 | CONTROLLER_STATE_TRANSITION | `createWorkbenchFlowService().submitCheckpointCorrection` and `.revealCheckpointAi` | real flowService, real `resolveTransition`, real workbenchStore/runtimeStore, real recallCheckpointService | typed in-memory repository fake; unused deps as typed tiny stubs | `in-memory repository fake` implementing actual repository methods invoked; `real production interface/type` for service deps | flowService under test; recallCheckpointService under test; runtime/workbench stores under test | Starting from recall/checkpoint_correction with active checkpoint and correctionDraft: submit persists correction line and clears draft; reveal persists AI lines/status, updates `recallSubstate='checkpoint_ai_revealed'`, keeps `mode='recall'`, keeps source Attempt protected fields unchanged. | P5CP-T05/P5CP-T07 |
| RED | P5CP-T03 | CONTROLLER_STATE_TRANSITION | `createWorkbenchFlowService().saveCheckpointComment` | real flowService, real workbenchStore/runtimeStore, real recallCheckpointService | typed in-memory repository fake; unused deps as typed tiny stubs | `in-memory repository fake`; `real production interface/type` | flowService under test; recallCheckpointService under test; repository state asserted via manual mutation outside command | From recall/checkpoint_ai_revealed, save creates `MoveComment` with checkpoint target, marks checkpoint commented/completed through resume, advances RecallSession, clears runtime active checkpoint, sets tab `recallSubstate='normal'`, and source Attempt `userLine/result/status` are unchanged. | P5CP-T05/P5CP-T07 |
| RED | P5CP-T04 | CONTROLLER_STATE_TRANSITION | `createWorkbenchFlowService().skipCheckpoint` | real flowService, real workbenchStore/runtimeStore, real recallCheckpointService | typed in-memory repository fake; unused deps as typed tiny stubs | `in-memory repository fake`; `real production interface/type` | flowService under test; recallCheckpointService under test | Skip marks checkpoint skipped/completed, advances RecallSession, clears `activeCheckpointId` and `correctionDraft`, sets tab `recallSubstate='normal'`, keeps `mode='recall'`, and source Attempt protected fields unchanged. | P5CP-T05/P5CP-T07 |
| RED | P5CP-T05 | RENDERED_UI_RETURN | `TrainingWorkbenchContainer` + `WorkbenchShell` + `RecallModePanel` rendered loop | real Container/Shell/Panel, real flowService, real stores, real recallCheckpointService | typed in-memory repository fake and unused typed deps | `in-memory repository fake`; `real production interface/type` | rendered components under test; flowService/checkpoint service under test; asserting only `container.render().props` while claiming rendered UI | Active checkpoint renders checkpoint controls; clicking rendered skip control runs full chain and re-renders with `activeCheckpointId=null`, `recallOriginalLine=true`, `recallSubstate='normal'`, and checkpoint controls absent or inactive. | not-covered: richer AI/comment visual details remain outside step1 |
| GREEN | P5CP-T06 | SERVICE_REPOSITORY_TRANSITION | `createRecallCheckpointService` service methods | real recallCheckpointService, real runtimeStore | existing typed/in-memory repository fake in service tests | existing service test harness bound to `TrainingRepository` interface | mocking recallCheckpointService while claiming service transition | Existing passing service tests prove correction/reveal/save/skip/resume repository/runtime effects; step1 tests may rely on this baseline but must still run real service in flow transition tests. | P5CP-T02/P5CP-T03/P5CP-T04 |
...
| 是否修改 source Attempt `userLine/result/status` | 否 | Implementation plan lines 813-815. | P5CP-T02/T03/T04 assert source Attempt protected fields unchanged。 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `submit-correction-btn` | `submitCheckpointCorrection(tabId)` | panel emits callback; Container delegates; flowService owns command; checkpointService owns persistence | Plan Phase 5 lines 797-799; Arch §9.5 lines 1842-1849 | runtime draft -> checkpoint userCorrectionLine; draft cleared; substate remains checkpoint_correction | runtime subscription -> `activeCheckpointId` remains; `recallOriginalLine=false`; reveal now allowed by flow guard | P5CP-T01, P5CP-T02 | tests by mode / controller |
| `查看 AI` button | `revealCheckpointAi(tabId)` | same | Plan Phase 5 lines 799, 823; Arch §9.5 lines 1851-1858 | checkpoint status ai_revealed; candidate lines persisted; tab substate checkpoint_ai_revealed | workbench subscription -> `recallSubstate`; runtime active checkpoint still projects checkpoint surface | P5CP-T01, P5CP-T02 | tests by mode / controller |
| `跳过 checkpoint` button | `skipCheckpoint(tabId)` | same | Plan Phase 5 lines 801-802; Arch state machine lines 147, 153-154 | checkpoint skipped/completed; RecallSession advanced; runtime active/draft cleared; tab substate normal | runtime/workbench subscriptions -> `activeCheckpointId=null`, `recallOriginalLine=true`, UI progress/original surface | P5CP-T01, P5CP-T04, P5CP-T05 | controller / rendered return |
| Shell prop `onSaveCheckpointComment({content})` | `saveCheckpointComment({tabId, content})` | Container delegates; flowService owns command; checkpointService owns save/resume | Plan Phase 5 lines 800-801, 824-825; Arch §9.5 lines 1860-1869 | MoveComment created; checkpoint commented/completed; RecallSession advanced; runtime active cleared; tab substate normal | subscriptions -> UI exits checkpoint surface | P5CP-T01, P5CP-T03 | controller |
...
| Visible comment editor/save button | deferred | future panel callback mapping | Phase 5 comment objective | not in current in-scope panel evidence | not-covered | P5CP-T08 deferred | future panel callback plumbing |
...
| --- | --- | --- | --- | --- | --- |
| submit checkpoint correction | callback only | activeTab guard + delegate | owns command, reads active checkpoint/draft, validates recall mode | `submitUserCorrectionLine` -> repository update, runtime draft clear | none |
| reveal checkpoint AI | callback only | activeTab guard + delegate | owns command, validates `revealAi` transition, updates substate | `revealAiCandidateLines` -> repository update | none |
| skip checkpoint | callback only | activeTab guard + delegate | owns command, updates substate normal after service success | `skipCheckpoint` -> checkpoint/session/runtime updates | none |
| save checkpoint comment | callback only / rendered UI deferred | activeTab guard + delegate with `{content}` | owns command, validates comment/resume transition, updates substate normal | `saveComment` + `resumeRecall` -> comment/checkpoint/session/runtime updates | none |
...
| DI integration | `src/modules/sabaki.js` if required to pass `checkpointService` into `createWorkbenchFlowService` | flowService deps finalized | Small isolated wiring patch | Not in original lock list; implementation worker must call out this expansion before editing. |
...
- Implementation must not mutate source Attempt `userLine`, `result`, or `status`; tests must snapshot or spy protected fields around real commands.
