...
window.sabaki = {
  ...
  humansl: {
    getModelInfo: () => ipcRenderer.invoke('humansl:getModelInfo'),
    ensureModel: () => ipcRenderer.invoke('humansl:ensureModel'),
  },
  ...
  db: {
    ...
    saveRecallSession: (session) =>
      ipcRenderer.invoke('db:saveRecallSession', session),
    saveRecallAttempts: (attempts) =>
      ipcRenderer.invoke('db:saveRecallAttempts', attempts),
    saveProblem: (problem) => ipcRenderer.invoke('db:saveProblem', problem),
    getProblem: (id) => ipcRenderer.invoke('db:getProblem', id),
    getProblemsByStatus: (status, limit) =>
      ipcRenderer.invoke('db:getProblemsByStatus', status, limit),
    saveProblemAttempt: (attempt) =>
      ipcRenderer.invoke('db:saveProblemAttempt', attempt),
    saveBadMove: (badMove) => ipcRenderer.invoke('db:saveBadMove', badMove),
    updateBadMoveGeneratedProblem: (badMoveId, problemId) =>
      ipcRenderer.invoke(
